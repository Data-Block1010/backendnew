...
void KYCVerification_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void KYCVerification_5_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "KYCVerification";
ctx->componentMemory[coffset].signalStart = soffset;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "nameCheck";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "idCheck";
IsEqual_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 6 ;
aux_cmp_num += 2;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "nationalityCheck";
IsEqual_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 6 ;
aux_cmp_num += 2;
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 14],&signalValues[mySignalStart + 15]); // line circom 30
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 6],&signalValues[mySignalStart + 5]); // line circom 34
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
KYCVerification_5_create(1,0,ctx,"main",0);
KYCVerification_5_run(0,ctx);
}
